...
class UserManager {
	...
	public function saveUser(User $user) {
		if ($user->getId() == -1) {
			$q = $this->bdd->prepare('INSERT INTO users SET lastname = :lastname, firstname = :firstname, login = :login, 
			email = :email, profil_id = :profil_id, password = :password, expiration = :expiration');
		} else {
			$q = $this->bdd->prepare('UPDATE users SET lastname = :lastname, firstname = :firstname, login = :login, 
